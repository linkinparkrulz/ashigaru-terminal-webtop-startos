...
ARG ASHIGARU_DEBVERSION
ARG ASHIGARU_PGP_SIG
...
RUN \
  ...
  # ashigaru requires this directory to exist
  mkdir -p /usr/share/desktop-directories/ && \
  # Find and install the Ashigaru Terminal .deb package
  DEB_FILE=$(ls /tmp/ashigaru_terminal_*_amd64.deb | head -1) && \
  echo "Installing Ashigaru Terminal from: $DEB_FILE" && \
  DEBIAN_FRONTEND=noninteractive \
  apt-get install -y "$DEB_FILE" && \
  # cleanup
  rm /tmp/ashigaru_terminal_*_amd64.deb
